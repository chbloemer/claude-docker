#!/usr/bin/env bash
#
# Smoke-test the claude-docker image: verify every tool the Dockerfiles install
# is present and (for pinned tools) at the expected version.
#
# Usage:
#   ./test.sh              # test the project image (service "claude")
#   ./test.sh base         # test the base image only (service "base")
#
# How it works: on the host this script pipes ITSELF into a container and runs
# the checks there. The container is detected via DEVCONTAINER=true (set in
# Dockerfile.base). The image entrypoint is bypassed (--entrypoint bash) so no
# OAuth prompt or Podman-socket startup interferes with the check.
...
# ─────────────────────────────────────────────────────────────────────────────
# HOST SIDE: re-run this script inside the container.
# ─────────────────────────────────────────────────────────────────────────────
if [ "${DEVCONTAINER:-}" != "true" ]; then
    ...
    exec $COMPOSE "${PROFILE[@]}" run --rm -T --entrypoint bash "$SERVICE" -s < "$0"
fi

# ─────────────────────────────────────────────────────────────────────────────
# CONTAINER SIDE: run the actual checks. (No set -e — we want every check to run.)
# ─────────────────────────────────────────────────────────────────────────────
